...
module.exports = (grunt) ->
  ...
  grunt.config.data.replace =
    dashed_paths:
      overwrite   : true
      replacements: [ from: /group-bot/g, to: "<%= pkg.name %>" ]
      src         : replace_paths = [
        "bin/*"
        "Gruntfile.coffee"
        "package.json"
    ...
    class_variables:
      overwrite   : true
      replacements: [ from: /GroupBot/g, to: "<%= grunt.util.toCamel(pkg.name) %>" ]
      src         : replace_paths
    instance_variables:
      overwrite   : true
      replacements: [ from: /group_bot/g, to: "<%= pkg.name.replace('-', '_') %>" ]
      src         : replace_paths
